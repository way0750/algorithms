...
// input an array
// let finalArr = []
// let reducedArr reduce the input array into [[ele], [ele, ele]]
//  make sure to compare each element to the last element in the last subarray in the reducedArr during reduce
// reducedArr should look like this: [[n], [n], [n,n,n,n], [n]];
// set while loop, loop while reduceArr has length of more than 1
// each loop reset reducerArr to merge two sub array together and push it into memo which is an array
// when done, return reducedArr[0];
...
//interactive way:
let mergeSort1 = (arr) => {
  let reducedArr = arr.reduce( (arr, ele) => {
    let lastSubArr = arr[arr.length - 1];
    lastSubArr[lastSubArr.length - 1] > ele ? arr.push([ele]) : lastSubArr.push(ele);
    return arr;
  }, [[]]);
  
  while (reducedArr.length > 1) {
    let tempSortedArrs = [];
    for (let i = 0; i < reducedArr.length; i+=2){
      let leftArr = reducedArr[i];
      let rightArr = reducedArr[i + 1]|| [];
      let zipArr = [];
      while (leftArr.length && rightArr.length) {
        let eleToPush = rightArr[0] > leftArr[0] ? leftArr.shift() : rightArr.shift();
        zipArr = zipArr.concat(eleToPush);
      }
      zipArr = zipArr.concat(leftArr, rightArr);
      tempSortedArrs.push(zipArr);
    }
    reducedArr = tempSortedArrs;
  }
  return reducedArr[0] || [];
};
